...
#     add_ax_title(ax, 'Playing Style')
    ...
    if team=='home':
        ax = fig.add_subplot(2, 6, n, projection='polar')
    ...
    params = ['Wing Play', 'Deep Play', 'Long Balls', 'Counter',
              'Crosses',
              'Key Passes', 'Low Block', 'High Press']
    
    # Set colors
    slice_colors = [TEAM_COLORS[team]]*len(params)
    #slice_colors = cmap([0.6, 0.7, 0.8, 0.9, 1])*8
    text_colors = ['w']*len(params)
    bg_colors = ['w']*len(params)
    ...
    baker.make_pizza(
        ...
        value_colors=['w']*len(params),        # color for the value-text
        ...
        kwargs_params=dict(
            color="w", fontsize=11, va="center", fontweight='bold'
