...
def plot_possession(fig, ax, df_home_poss, df_away_poss, added_time: list):
    ...
    # ax.set_yticks([])
    # ax.tick_params(axis="both", which="both", length=0, colors="#ededed", labelsize=14)
...
def plot_momentum(fig, ax, df_home_poss, df_away_poss, home_goals, away_goals, added_time: list):
    ...
    # Shading around line
    ax.plot(x, y_momentum,
       color=home_color, lw=7, alpha=0.2)
...
#     ax.grid(False)
    ...
    ax.grid(ls="-", lw=.5, color="#131313", axis='both', zorder=1)
    spines = ['top','right']
    for x in spines:
        if x in spines:
...
#             fontsize=12, ha='center')
    ...
    ax.scatter(x=home_goals, y=home_y, s=700, facecolor=home_color, edgecolor='w', marker="*")
...
#     imscatter(x=away_goals, y=away_y, image='utils/football.png', ax=ax, zoom=0.1)
    ax.scatter(x=away_goals, y=away_y, s=700, facecolor=away_color, edgecolor='w', marker="*")
    ...
    # Add title
    add_ax_title(ax, 'Possession percentage')
