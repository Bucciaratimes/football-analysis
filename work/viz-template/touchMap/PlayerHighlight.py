...
class PlayerHighlight:
    ...
    def __init__(self, pitch, axes, team, team_id, league, season, gw, player_id, venue, fig_color):
    ...
    def plot_passmap(self, df, ax_num, time:int=None)->int:
        ...
        self.axes[ax_num].fill_between((45,60), (85, 85), (120, 120),color="#fb5607",alpha=.3)
        self.axes[ax_num].fill_between((20,35), (85, 85), (120, 120),color="#8338ec",alpha=.3)
        ...
        for _, row in player_df.iterrows():
            ...
            #     )
                ...
                if 'passAccurate' in row["satisfiedEventsTypes"]:
                    ...
                    # lines = self.pitch.lines(
                    #         xstart=row["x"], ystart=row["y"],
                    #         xend=row["endX"], yend=row["endY"],
                    #         cmap=cmap, 
                    #         comet=False,
                    #         linewidth=2,
                    #         ax=self.axes[ax_num],
    ...
    def plot_shotmap(self, pitch, df, ax_num):
        ...
        player_df = df[df["playerId"] == self.player_id]
        # player_df["endX"].fillna(120, inplace=True)
        # player_df["endY"].fillna(40, inplace=True)
    ...
    def plot_carry(self, df, ax_num):
        ...
        df = self.add_xT(df)
        df = df.query(f"carrydist>={min_carry_dist} and playerId=={self.player_id}").dropna()
    ...
    def plot_note(self, fig, whoscore_data, passes, sorted_column=None, carries=None, shots=None):
        ...
        if self.venue == "home":
            ...
            fig_text(
                s=f"<{self.opponent.title()} vs {self.team.title()} | {self.note_score} | {self.note_league} | {self.note_venue_name} | whoscored.com | @Bucciaratimes >",
                #| table is sorted by {sorted_column} >",
                x=self.axes[0].get_position().x0 + .045,
                y=.89,
                color="#ffffff",
        ...
        fig_text(
            ...
            s=f"<Accurate pass>     {rs[0]}({rs[2]}%)     <Inaccurate pass>     {rs[1]}({round(100-rs[2], 2)}%) \n  <Key pass>     {passes}                         <Dribble>    {carries}               <Shot>    {shots[0]}",    
            va="bottom", ha="center",
            color="#171717",
            fontproperties=font_prop,
            weight="semibold",
    ...
    def make_xTGraph_data(self, df):
        ...
        carry_df = carry_df.query(f"carrydist>={min_carry_dist}").dropna()
    ...
    def plot_xTBar(self, ax_num, xT, players):
        ...
        self.axes[ax_num].hlines(y=y_value, xmin=0, xmax=x_value, color=color, alpha=0.7, linewidth=1.5, zorder=1)
        self.axes[ax_num].scatter(
            x_value, y_value, 
        ...
        if min(x_value) < 0:
            self.axes[ax_num].spines['left'].set_position(('data', min(x_value)+(min(x_value))))
        else:
            self.axes[ax_num].spines['left'].set_position(('data', min(x_value)-(min(x_value))))
        self.axes[ax_num].spines['left'].set_visible(False)
        ...
        y_labels = self.axes[ax_num].set_yticklabels(players, fontproperties=font_prop, fontsize=12, color="#010101",fontweight="bold")
        # for idx, y_label in enumerate(y_labels):
        #     y_label.set_position(xy=(min(x_value)-.001, self.axes[ax_num].get_yticks()[idx]))
        # self.axes[ax_num].set_yticks(ticks=self.axes[ax_num].get_yticks(), labels=players, fontproperties=font_prop, fontsize=12, color="#010101",fontweight="bold")
    ...
    def plot_xTGraph(self, df, ax_num):
        ...
        value_pass = xTPie_data.iloc[0]
        value_dribble = xTPie_data.iloc[1]
        if value_pass  < 0:
            value_pass  = abs(value_pass)
        if value_dribble < 0:
            value_dribble = abs(value_dribble)
        
        self.plot_xTPie(ax_num+2, [value_pass, value_dribble], categories1)
...
def main(theme:str):
    ...
    carry_count = player_highlight.plot_carry(whoscore_data, 0)
...
if __name__ == "__main__":
    ...
    if skip == "y":
        ...
        highlight_team = input("highlight_team ...? ")
        highlight_league = input("highlight_league ...? ")
        highlight_season = input("season: ....?")
        highlight_gw = input("gameweek: ....?")
        highlight_player_id = int(input("player_id: ....?"))
        venue = input("home or away: ....?")
        club_icon_id = int(input("club_icon...? "))
    
    fotmob_url = 'https://images.fotmob.com/image_resources/logo/teamlogo/'
    
    highlight_mark = Image.open(urllib.request.urlopen(f'{fotmob_url}{club_icon_id:.0f}.png')).convert('RGBA') # 白黒：LA, カラフル：RGBA
