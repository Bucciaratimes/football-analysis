...
def getLeagueUrls(minimize_window=True):
    ...
    league_urls = []
    for i in range(21):
        league_name = driver.find_element_by_xpath(
            '//*[@id="popular-tournaments-list"]/li[' + str(i + 1) + ']/a').text
...
def getMatchUrls(comp_urls, competition, season, maximize_window=True):
    ...
    seasons = driver.find_element_by_xpath(
        '//*[@id="seasons"]').get_attribute('innerHTML').split(sep='\n')
    seasons = [i for i in seasons if i]

    for i in range(1, len(seasons) + 1):
        if driver.find_element_by_xpath(
                '//*[@id="seasons"]/option[' + str(i) + ']').text == season:
            driver.find_element_by_xpath(
                '//*[@id="seasons"]/option[' + str(i) + ']').click()

            time.sleep(5)
            try:
                stages = driver.find_element_by_xpath(
                    '//*[@id="stages"]').get_attribute('innerHTML').split(sep='\n')
                ...
                for i in range(1, len(stages) + 1):
                    if competition == 'Champions League' or competition == 'Europa League':
                        if 'Group Stages' in driver.find_element_by_xpath(
                                '//*[@id="stages"]/option[' +
                                str(i) +
                                ']').text or 'Final Stage' in driver.find_element_by_xpath(
                                '//*[@id="stages"]/option[' +
                                str(i) +
                                ']').text:
                            driver.find_element_by_xpath(
                                '//*[@id="stages"]/option[' + str(i) + ']').click()
                            time.sleep(5)
                        ...
                        if 'Grp. ' not in driver.find_element_by_xpath(
        ...
        for row in table_rows:
            ...
            if type(link_tag) is type(None):
                if type(
                    element.find(
                        'span', {
                            'class': 'status-1 rc'})) is type(None):
                    date = row.text.split(', ')[-1]
            if type(link_tag) is not type(None):
                match_dict['date'] = date
                match_dict['time'] = element.find(
                    'div', {
                        'class': 'col12-lg-1 col12-m-1 col12-s-0 col12-xs-0 time divtable-data'}).text
                match_dict['home'] = element.find_all(
                    "a", {"class": "team-link"})[0].text
                match_dict['away'] = element.find_all(
                    "a", {"class": "team-link"})[1].text
                match_dict['score'] = element.find(
                    "a", {"class": "result-1 rc"}).text
                match_dict['url'] = link_tag.get("href")
            matches_ls.append(match_dict)

        prev_month = driver.find_element_by_xpath(
            '//*[@id="date-controller"]/a[1]').click()
        time.sleep(2)
        if driver.find_element_by_xpath(
                '//*[@id="date-controller"]/a[1]').get_attribute('title') == 'No data for previous week':
            table_rows = driver.find_elements_by_class_name('divtable-row')
            for row in table_rows:
                match_dict = {}
                element = soup(row.get_attribute('innerHTML'), features='lxml')
...
def getMatchData(driver, url, display=True, close_window=True):
    ...
    script_content = driver.find_element_by_xpath(
        '//*[@id="layout-wrapper"]/script[1]').get_attribute('innerHTML')
    ...
    region = driver.find_element_by_xpath(
        '//*[@id="breadcrumb-nav"]/span[1]').text
    league = driver.find_element_by_xpath(
        '//*[@id="breadcrumb-nav"]/a').text.split(' - ')[0]
    season = driver.find_element_by_xpath(
        '//*[@id="breadcrumb-nav"]/a').text.split(' - ')[1]
    if len(driver.find_element_by_xpath(
        ...
        competition_stage = driver.find_element_by_xpath(
            '//*[@id="breadcrumb-nav"]/a').text.split(' - ')[-1]
    else:
...
def createEventsDF(data):
    ...
    for i in range(len(events_df)):
        ...
        events_df['satisfiedEventsTypes'].loc[i] = [
            list(
                eventTypeDict.keys())[
                list(
                    eventTypeDict.values()).index(event)] for event in row]
    ...
    events_df.loc[events_df.playerId.notna(),
                  'playerId'] = events_df.loc[events_df.playerId.notna(),
                                              'playerId'].astype(int).astype(str)
    ...
    for i in events_df.loc[events_df.isShot].index:
        for j in events_df.loc[events_df.isShot].qualifiers.loc[i]:
    ...
    for event_type in event_types:
        events_df[event_type] = pd.Series(
            [event_type in row for row in list(events_df['satisfiedEventsTypes'])])
...
def write_date(season):
    urls = get_today_matchUrls()
    matches = getMatchesData(urls)
    for i,match_data in enumerate(matches):
        ...
        events_df['home'] = homeName.title()
        events_df['away'] = awayName.title()
        path = f"/work/assets/whoscored/automation/match/{season}/eventsData/new/"
        if not os.path.exists(path):
            os.makedirs(path)
        events_df.to_csv(path + f'{season}#{i+1}.csv')
        path = f"/work/assets/whoscored/automation/match/{season}/matchData/"
        if not os.path.exists(path):
            os.makedirs(path)
