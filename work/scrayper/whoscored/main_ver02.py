...
def getLeagueUrls(minimize_window=True):
    ...
    league_urls = []
    for i in range(1,21):
        league_name = driver.find_element_by_xpath(f'//*[@id="popular-tournaments-list"]/li[{i}]/a').text
        league_link = driver.find_element_by_xpath(f'//*[@id="popular-tournaments-list"]/li[{i}]/a').get_attribute('href')
...
def getMatchUrls(comp_url, season, maximize_window=True):
    ...
    for i in range(1, len(seasons)+1):
        if driver.find_element_by_xpath(f'//*[@id="seasons"]/option[{i}]').text == season:
            season = driver.find_element_by_xpath(f'//*[@id="seasons"]/option[{i}]').click()
...
def getMatchesData(match_urls, minimize_window=True):
    ...
    try:
        for i in trange(len(match_urls), desc='Getting Match Data'):
            ...
            time.sleep(7)
            match_data = getMatchData(main_url+match_urls[i]['url'], display=False, close_window=False)
...
def getMatchData(url, display=True, close_window=True):
