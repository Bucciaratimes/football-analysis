...
def getAllLinksTeam(baseURL):
    ...
    links = []
    time.sleep(2)
    matchesList = driver.find_elements_by_xpath(
        "//*[@class='horiz-match-link result-1']")
...
def getJSON(driver, link):
    ...
    driver.get(link)
    time.sleep(10)
    raw_result = driver.find_elements_by_xpath(
        ("//*[@id='layout-wrapper']/script[1]"))[0]
    ...
    formationIdNameMappings = raw_result[raw_result.find(
        "formationIdNameMappings"):]
    formationIdNameMappings = formationIdNameMappings[formationIdNameMappings.find(
        "{"):formationIdNameMappings.find(";")]
...
def getAllTeamsByLeague(path):
    ...
    driver.get(path)
    time.sleep(2)
    teamsList = driver.find_elements_by_xpath(
        "//*[starts-with(@id, 'standings-')]/tr[*]/td[*]/a[@class='team-link ']")
    i = 0
