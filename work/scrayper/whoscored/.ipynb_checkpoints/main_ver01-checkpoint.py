...
#   1617->13955 1718->15375 1819->16546 1920->17702 
    ...
    ## add: 2021/12/25 to get season 
    dropdown = driver.find_element_by_id('seasons') # ② select要素を取得
    select = Select(dropdown)
    select.select_by_index(0) # 0:2122, 1:2021 
    ############################
...
#             serie -> standings-18873-content
    ...
    time.sleep(5)
    #fixtures_page = driver.find_element(
    #    '//*[@id="link-fixtures"]').click()
    
    link_fixtures = driver.find_elements_by_id("link-fixtures")[0]
    link_fixtures.click()
    
    time.sleep(5)
    #date_config_btn = driver.find_element(
    ...
    selectable_months = driver.find_element('xpath',
    ...
    for i in range(n_months):
        time.sleep(2)
        fixtures_table = driver.find_element('xpath','//*[@id="tournament-fixture"]')
...
#         links = [main_url+row.find("a", {"class":"result-4 rc"}).get("href") for row in table_rows]  
        ...
        previous_month = driver.find_element('xpath','//*[@id="date-controller"]/a[1]').click()
...
def getTeamData(team_links):
    ...
    for i in trange(len(team_links), desc='Single loop'):
        ...
        script_content = driver.find_element('//*[@id="layout-wrapper"]/script[1]').get_attribute('innerHTML')
...
def getUnderstatShotData(match_url, driver):
    ...
    shot_data_tag = driver.find_element('xpath','/html/body/div[1]/div[3]/div[2]/div[1]/div/script')
...
def getxGFromUnderstat(match_data, events_df, driver):
    ...
    und_leagues = driver.find_element('xpath','//*[@id="header"]/div/nav[1]/ul').text.split('\n')
    found = False
    ...
    if found == False:
        ...
        season_btn = driver.find_element('xpath','//*[@id="header"]/div/div[2]/div').click()
        und_seasons = driver.find_element('xpath','//*[@id="header"]/div/div[2]/ul').text.split('\n')
        found = False
        for szn in und_seasons:
            if match_data['season'] == szn:
                i = str(und_seasons.index(szn)+1)
                driver.find_element('xpath',"//*[@id='header']/div/div[2]/ul/li["+i+"]").click()
                found = True
                break
        ...
        if found == False:
            ...
            timezn_off_btn = driver.find_element('xpath','/html/body/div[1]/div[3]/div[2]/div/div/div[1]/div/label[3]').click()
            date = '-'.join(match_data['startDate'].split('T')[0].split('-')[::-1])
            d = datetime.datetime.strptime(date, '%d-%m-%Y')
            date = datetime.date.strftime(d, "%A, %B %d, %Y")
            prev_btn = driver.find_element('xpath','/html/body/div[1]/div[3]/div[2]/div/div/button[1]')
            next_btn = driver.find_element('xpath','/html/body/div[1]/div[3]/div[2]/div/div/button[2]')
            btn_ls = []
            found = True
            while found:
