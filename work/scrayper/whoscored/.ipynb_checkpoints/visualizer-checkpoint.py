...
def createShotmap(
    ...
    total_shots = events_df.loc[[9 in row for row in list(
        events_df['satisfiedEventsTypes'])]]
    
    team_shots = total_shots.loc[total_shots['teamId']
                                 == teamId].reset_index(drop=True)
    
    mask_goal = team_shots['isGoal']
    ...
    # Plot the goals
    pitch.scatter(
        team_shots[mask_goal]['x'] /
        100 *
        120,
        80 -
        team_shots[mask_goal]['y'] /
        100 *
        80,
        s=marker_size,
        edgecolors='black',
        ...
        label='goal',
        ax=ax)
    pitch.scatter(team_shots[~mask_goal].x / 100 * 120,
                  80 - team_shots[~mask_goal].y / 100 * 80,
                  edgecolors='white',
                  c=shotcolor,
                  s=marker_size,
                  zorder=2,
...
def createPassNetworks(matches_df, events_df, matchId, teamId, team, opponent, venue,
    ...
    pitch = Pitch(pitch_type='statsbomb', orientation='horizontal',
                  pitch_color=pitch_color, line_color='#c7d5cc', figsize=(13.5, 8),
    ...
    plt.savefig(f'/output/barcelona/PassNetwork_{opponent}.png', facecolor=fig.get_facecolor(), edgecolor='none')
...
def getTeamSuccessfulBoxPasses(events_df, teamId, team, pitch_color, cmap):
    ...
    # Get Total Passes
    passes_df = events_df.loc[[row['displayName'] == 'Pass' for row in list(
        events_df['type'])]].reset_index(drop=True)
    ...
    pitch.lines(
        successful_box_passes.x / 100 * 120,
        80 - successful_box_passes.y / 100 * 80,
        successful_box_passes.endX / 100 * 120,
        80 - successful_box_passes.endY / 100 * 80,
        lw=5,
        opp_transparent=True,
        opp_comet=True,
        cmap=cmap,
    ...
    pitch.scatter(
        successful_box_passes.x /
        100 *
        120,
        80 -
        successful_box_passes.y /
        100 *
        80,
        edgecolors='white',
        c='white',
    ...
    # set legend
    # ax.legend(facecolor='#22312b', edgecolor='None', fontsize=8, loc='lower
    # center', handlelength=4)
